...
#if defined(NDEBUG)
# error "reedcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf3e852d924c7a874c585613fa6675a279a00a744c4cc230e5c224ee5c6a14904");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // reedcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "reedcoin Signed Message:\n";
...
{
    // reedcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // reedcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // reedcoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // reedcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // reedcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // reedcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9aa83e23cff63de0e719a89cbc9a9446366a3022227766ec0b169953170b3927");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "23/mar/2021 - Reedcoin creation.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bfe3aa505d71b80c466856eeab53434ffcf606469590c192569b84db89d2911f4574e94af92cbc03cd17b764ec0d3410ef81c6dbdc72cc0d4eb8c86f92141917") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616877105;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084806428;

        if (fTestNet)
        {
            block.nTime    = 1616877087;
            block.nNonce   = 385989379;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf1297829d95a545ea22522b3602c5e8cccff94ad323a79a9911c5b504792dc59"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xcb, 0xa9, 0x8a }; // reedcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("reedcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("reedcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static reedcoinMiner(CWallet *pwallet)
{
    printf("reedcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reedcoin-miner");
    ...
    try { loop {
        ...
        printf("Running reedcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("reedcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&reedcoinMiner, pwallet));
}
